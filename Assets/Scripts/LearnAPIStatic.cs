...
namespace Julee
{
    ...
    public class LearnAPIStatic : MonoBehaviour
    {
        private void Awake()
        {
            ...
            // Random.value = 0.5f;     // Read Only 唯獨屬性不能設定

            // 靜態屬性 Static Methods
            // 3. 使用靜態方法
            // 語法:
            // 類別名稱.靜態屬性名稱(對應的引數); 
            print($"<color=#3399ff>隨機攻擊 100.5f ~ 150.9 : { Random.Range(100.5f, 150.9f) }</color>");

            float random = Random.Range(10.9f, 20.5f);
            print($"<color=#3366ff>隨機攻擊 10.9f ~ 20.5 : { random }</color>");
        }
        ...
        private void Update()
        {
            ...
            print($"<color=#6699ff>是否按下空白建: {space}</color>");
        }
    }
}
